...
export async function loadJatosScriptCandidates(
  candidates: string[] = DEFAULT_JATOS_SCRIPT_CANDIDATES,
): Promise<JatosScriptLoadResult> {
...
export async function waitForJatosReady(timeoutMs = 2000): Promise<void> {
...
export async function resolveSelectionWithJatosRetry(
  coreConfig: CoreConfig,
  maxWaitMs = 10000,
): Promise<SelectionContext> {
  if (typeof window === "undefined") return resolveSelection(coreConfig);
