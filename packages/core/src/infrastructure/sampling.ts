...
export interface SamplerBackend {
  ...
  truncnorm?: (args: { mu: number; sd: number; min?: number; max?: number }, rng: SamplerRng) => number;
  exponential?: (lambda: number, rng: SamplerRng) => number;
  poisson?: (lambda: number, rng: SamplerRng) => number;
  negativeBinomial?: (args: { mu: number; k: number }, rng: SamplerRng) => number;
}
...
export const SAMPLER_TYPES = Object.freeze<SamplerType[]>([
  "fixed",
  "uniform",
  "normal",
  "truncnorm",
...
const isCreateSamplerOptions = (value: unknown): value is CreateSamplerOptions =>
  isObject(value) && (Object.prototype.hasOwnProperty.call(value, "rng") || Object.prototype.hasOwnProperty.call(value, "backend"));

const toSamplerRng = (rng?: SamplerRng): SamplerRng => rng ?? defaultRng;

const nextFloat = (rng: SamplerRng): number => {
...
const nextRange = (rng: SamplerRng, min: number, max: number): number => {
  if (typeof rng.nextRange === "function") return rng.nextRange(min, max);
  return min + (max - min) * nextFloat(rng);
};

const nextNormal = (rng: SamplerRng, mu = 0, sigma = 1): number => {
  if (typeof rng.nextNormal === "function") return rng.nextNormal(mu, sigma);
  let u1 = 0;
...
const samplePoissonWithLambda = (lambda: number, rng: SamplerRng, backend?: SamplerBackend): number => {
  if (backend?.poisson) return backend.poisson(lambda, rng);
  if (lambda <= 50) {
...
const sampleGamma = (shape: number, scale: number, rng: SamplerRng): number => {
  if (!(shape > 0) || !(scale > 0)) {
...
const shuffleInPlace = <T>(items: T[], rng: SamplerRng): T[] => {
...
const parseListWeights = (weightsSpec: unknown, itemCount: number): number[] | null => {
  if (weightsSpec == null) return null;
...
const normalizeSamplerSpec = (spec: SamplerSpec): SamplerSpecObject => {
...
function drawNormal(mu: number, sd: number, rng: SamplerRng, backend?: SamplerBackend): number {
  return backend?.normal ? backend.normal(mu, sd, rng) : nextNormal(rng, mu, sd);
}

function createDefaultSeededRng(seed = 1): SamplerRng {
...
export function createSampler(spec: SamplerSpec, options?: CreateSamplerOptions): () => unknown;
export function createSampler(spec: SamplerSpec, rng?: SamplerRng, backend?: SamplerBackend): () => unknown;
export function createSampler(
  spec: SamplerSpec,
  optionsOrRng?: CreateSamplerOptions | SamplerRng,
  backendArg?: SamplerBackend,
): () => unknown {
  const options: CreateSamplerOptions = isCreateSamplerOptions(optionsOrRng)
    ? optionsOrRng
    : { rng: optionsOrRng as SamplerRng | undefined, backend: backendArg };
  ...
  switch (type) {
    ...
    case "list": {
      ...
      const withoutReplacement =
        normalizedSpec.without_replacement === true ||
...
export function sampleValue(spec: SamplerSpec, options?: CreateSamplerOptions): unknown;
export function sampleValue(spec: SamplerSpec, rng?: SamplerRng, backend?: SamplerBackend): unknown;
