...
function extractStringLike(value) {
    ...
    if (!obj)
        return null;
    return (pickFirstString(obj, ["id", "name", "key", "value", "taskId", "task", "variantId", "variant", "experiment"])
        ?? null);
}
function pickFirstSelectionValue(source, keys) {
    if (!source)
        return null;
    for (const key of keys) {
