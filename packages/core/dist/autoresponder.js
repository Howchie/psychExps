import { createMulberry32, hashSeed } from "./random";
...
function resolveResponseRt(value, fallback) {
    const input = asObject(value);
    if (!input)
        return fallback;
    const minMs = Math.max(0, Math.round(toFiniteNumber(input.minMs, fallback.minMs)));
...
function sampleTruncatedNormal(mean, sd, min, max, random) {
    for (let i = 0; i < 8; i += 1) {
...
function sampleUniformMs(range, random) {
...
export function configureAutoResponder(profile) {
    if (!profile.enabled) {
        activeProfile = null;
        rng = null;
        return;
...
export function isAutoResponderEnabled() {
    return !!activeProfile?.enabled;
}
export function getAutoResponderProfile() {
    return activeProfile;
}
export function sampleAutoContinueDelayMs() {
    if (!activeProfile)
        return null;
...
export function sampleAutoResponse(args) {
    ...
    if (validResponses.length === 0)
        return { response: null, rtMs: null };
    if (random() < activeProfile.timeoutRate)
        return { response: null, rtMs: null };
    ...
    const sampledRt = sampleTruncatedNormal(activeProfile.responseRtMs.meanMs, activeProfile.responseRtMs.sdMs, activeProfile.responseRtMs.minMs, activeProfile.responseRtMs.maxMs, random);
...
export function sampleAutoInteractionDelayMs() {
    if (!activeProfile)
        return null;
    return sampleUniformMs(activeProfile.interActionDelayMs, random);
}
export function sampleAutoHoldDurationMs() {
    if (!activeProfile)
        return null;
