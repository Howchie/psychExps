...
const asFinitePositive = (value, fallback) => {
...
export function createConditionCellId(levels) {
    const pairs = Object.entries(levels).sort(([a], [b]) => a.localeCompare(b));
    return pairs.map(([key, value]) => `${key}=${value}`).join("|");
}
export function enumerateConditionCells(factors) {
    ...
    for (const factor of normalized) {
        if (!factor.name)
            throw new Error("Condition factor name cannot be empty.");
...
function violatesAdjacency(sequence, candidate, rules) {
    if (!rules || sequence.length === 0)
        return false;
    ...
    if (rules.maxRunLengthByCell && rules.maxRunLengthByCell > 0) {
        ...
        for (let i = sequence.length - 1; i >= 0; i -= 1) {
            if (sequence[i].id !== candidate.id)
                break;
            runLength += 1;
            if (runLength > rules.maxRunLengthByCell)
                return true;
        }
    }
    if (rules.maxRunLengthByFactor) {
        for (const [factorName, maxRun] of Object.entries(rules.maxRunLengthByFactor)) {
            ...
            let runLength = 1;
            for (let i = sequence.length - 1; i >= 0; i -= 1) {
                if (sequence[i].levels[factorName] !== nextValue)
                    break;
                runLength += 1;
                if (runLength > limit)
                    return true;
...
function buildCandidatePool(cells, counts) {
    const output = [];
...
export function buildConditionSequence(args) {
