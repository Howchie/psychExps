...
export function normalizeDrtKey(value) {
...
export class DrtEngine {
    ...
    step(nowMs, hooks) {
        ...
        if (!this.activeStim)
            return;
        if (this.activeStim.responded)
            return;
        const latency = nowMs - this.activeStim.start;
        if (latency < this.responseDeadlineMs)
            return;
        this.stats.misses += 1;
...
export function coerceScopedDrtConfig(base, overrideRaw) {
    if (!overrideRaw)
        return { ...base };
    ...
    return {
        ...
        transformPersistence: (asString(overrideRaw.transformPersistence ?? overrideRaw.transform_persistence) || base.transformPersistence) === "session"
            ? "session"
            : "scope",
        responseWindowMs: toPositiveNumber(overrideRaw.responseWindowMs ?? overrideRaw.response_window_ms ?? overrideRaw.responseDeadlineMs ?? overrideRaw.response_deadline_ms, base.responseWindowMs),
        displayDurationMs: toPositiveNumber(overrideRaw.displayDurationMs ?? overrideRaw.display_duration_ms, base.displayDurationMs),
        responseTerminatesStimulus: overrideRaw.responseTerminatesStimulus === undefined && overrideRaw.response_terminates_stimulus === undefined
        ...
        parameterTransforms: asArray(overrideRaw.parameterTransforms ?? overrideRaw.parameter_transforms ?? overrideRaw.transforms ?? base.parameterTransforms).flatMap((entry) => {
        ...
        audio: {
            ...
            ...(audioRaw
                ? {
                    ...
                    waveform: (asString(audioRaw.waveform) || base.audio?.waveform || "sine"),
                }
                : {}),
        },
        border: {
...
function expandStimMode(stimMode) {
    const mode = String(stimMode ?? "visual").toLowerCase();
    if (mode === "audiovisual")
        return new Set(["visual", "auditory"]);
    if (mode === "visual_border")
        return new Set(["visual", "border"]);
    if (mode === "auditory_border")
        return new Set(["auditory", "border"]);
    if (mode === "all")
        return new Set(["visual", "auditory", "border"]);
    if (mode === "visual" || mode === "auditory" || mode === "border")
        return new Set([mode]);
    return new Set(["visual"]);
}
function hasMode(config, mode) {
...
export function createDrtPresentationBridge(config, adapter) {
    ...
    const hideVisual = (stimulus) => {
        if (!hasVisualMode)
            return;
        adapter.hideVisual?.(stimulus);
    };
    const hideBorder = (stimulus) => {
        if (!hasBorderMode)
            return;
    ...
    return {
        ...
        onStimStart: (stimulus) => {
            if (hasAuditoryMode)
                adapter.playAuditory?.(stimulus);
            if (hasVisualMode)
                adapter.showVisual?.(stimulus);
            if (hasBorderMode)
                adapter.showBorder?.(stimulus);
        },
        onStimEnd: (stimulus) => {
            hideVisual(stimulus);
            hideBorder(stimulus);
...
function normalizeControllerModes(config) {
    if (Array.isArray(config.stimModes) && config.stimModes.length > 0) {
    ...
    if (mode === "auditory")
        return new Set(["auditory"]);
    if (mode === "border")
        return new Set(["border"]);
    if (mode === "audiovisual")
        return new Set(["visual", "auditory"]);
    if (mode === "visual_border")
        return new Set(["visual", "border"]);
...
function createNextIsiMs(config) {
    if (typeof config.nextIsiMs === "function")
        return config.nextIsiMs;
    const rng = new SeededRandom(Math.floor(Number(config.seed ?? 1)) >>> 0);
    const sampler = createSampler(config.isiSampler ?? { type: "uniform", min: 3000, max: 5000 }, { rng: { next: () => rng.next() } });
    return () => toPositiveNumber(sampler(), 5000);
}
function normalizeControllerConfig(config) {
...
export class DrtPresenter {
    ...
    borderTargetRect;
    visualElement = null;
    borderOverlayElement = null;
    audioContext = null;
    constructor(config, displayElement, borderTargetElement, borderTargetRect) {
        this.config = config;
        this.displayElement = displayElement;
        this.borderTargetElement = borderTargetElement;
        this.borderTargetRect = borderTargetRect;
    }
    showPresentation(activePresentation) {
        if (!activePresentation?.visible)
            return;
        if (this.config.modes.has("visual"))
            this.showVisual();
        if (this.config.modes.has("border"))
            this.showBorder();
        if (this.config.modes.has("auditory"))
            this.playTone();
    }
    hidePresentation(activePresentation) {
        if (!activePresentation || !activePresentation.visible)
            return;
    ...
    ensureBorderOverlayElement() {
        if (this.borderOverlayElement)
            return this.borderOverlayElement;
        if (typeof document === "undefined")
            return null;
        const element = document.createElement("div");
        element.style.position = "fixed";
...
export class DrtController {
    ...
    constructor(config, hooks = {}, options = {}) {
    ...
    static asTaskModule(config) {
        return {
            id: "drt",
            start: (moduleConfig, address, context) => {
                const controller = new DrtController({ ...moduleConfig, enabled: true }, {}, {
    ...
    handleEngineEvent(event) {
        ...
        if (!this.transformRunner?.isEnabled())
            return;
        if (event.type !== "drt_response")
            return;
    ...
    tickPresentationTimeout() {
        const active = this.activePresentation;
        if (!active || !active.visible)
            return;
        if (this.elapsedNowMs() < active.hideAtMs)
            return;
        this.hidePresentation();
    }
}
