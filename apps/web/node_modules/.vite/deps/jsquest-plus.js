...
// ../../node_modules/numeric_es6/src/numeric.js
var numeric = {};
...
numeric.parseCSV = function parseCSV(t2) {
  var foo = t2.split("\n");
  var j, k2;
  var ret = [];
  var pat = /(([^'",]*)|('[^']*')|("[^"]*")),/g;
...
numeric.dotMV = function dotMV(x, y) {
  ...
  var ret = Array(p), dotVV3 = numeric.dotVV;
  for (i = p - 1; i >= 0; i--) {
    ret[i] = dotVV3(x[i], y);
  }
  return ret;
...
numeric.T.prototype.reciprocal = function reciprocal() {
  var mul = numeric.mul, div2 = numeric.div;
  if (this.y) {
    var d = numeric.add(mul(this.x, this.x), mul(this.y, this.y));
    return new numeric.T(div2(this.x, d), div2(numeric.neg(this.y), d));
  }
  return new T(div2(1, this.x));
};
...
numeric.T.prototype.transpose = function transpose2() {
  var t2 = numeric.transpose, x = this.x, y = this.y;
  if (y) {
    return new numeric.T(t2(x), t2(y));
  }
  return new numeric.T(t2(x));
};
numeric.T.prototype.transjugate = function transjugate() {
  var t2 = numeric.transpose, x = this.x, y = this.y;
  if (y) {
    return new numeric.T(t2(x), numeric.negtranspose(y));
  }
  return new numeric.T(t2(x));
};
...
numeric.T.prototype.inv = function inv2() {
  var A2 = this;
...
numeric.T.prototype.getBlock = function getBlock2(from, to) {
...
numeric.T.prototype.setBlock = function setBlock2(from, to, A2) {
...
numeric.T.rep = function rep2(s, v) {
  var T3 = numeric.T;
  if (!(v instanceof T3)) v = new T3(v);
  var x = v.x, y = v.y, r = numeric.rep;
  if (y) return new T3(r(s, x), r(s, y));
  return new T3(r(s, x));
};
numeric.T.diag = function diag2(d) {
  if (!(d instanceof numeric.T)) d = new numeric.T(d);
  var x = d.x, y = d.y, diag4 = numeric.diag;
  if (y) return new numeric.T(diag4(x), diag4(y));
  return new numeric.T(diag4(x));
};
...
numeric.T.identity = function identity2(n) {
...
numeric.QRFrancis = function(H, maxiter) {
  ...
  var s = numeric.dim(H), m = s[0], x, v, a, b, c, d, det2, tr, Hloc, Q = numeric.identity(m), Qi, Hi, B2, C2, Ci, i, j, k2, iter;
  if (m < 3) {
    return { Q, B: [[0, m - 1]] };
  }
  ...
  for (iter = 0; iter < maxiter; iter++) {
    ...
    if (tr * tr >= 4 * det2) {
      var s1, s2;
      s1 = 0.5 * (tr + Math.sqrt(tr * tr - 4 * det2));
      s2 = 0.5 * (tr - Math.sqrt(tr * tr - 4 * det2));
      Hloc = numeric.add(
        numeric.sub(
      ...
      Hloc = numeric.add(
        ...
        numeric.diag(numeric.rep([3], det2))
      );
    }
...
numeric.eig = function eig2(A2, maxiter) {
  ...
  var Q = new T3(numeric.dot(QB.Q, QH.Q)), Q0;
  var m = B2.length, j;
...
numeric.ccsTSolve = function ccsTSolve(A2, b, x, bj, xj) {
  ...
  function dfs3(j2) {
    ...
    if (x[j2] !== 0) return;
    x[j2] = 1;
    for (k3 = Ai[j2]; k3 < Ai[j2 + 1]; ++k3) dfs3(Aj[k3]);
    xj[n] = j2;
    ++n;
  }
...
numeric.ccsLPSolve = function ccsLPSolve(A2, B2, x, xj, I, Pinv, dfs3) {
  ...
  xj.length = 0;
  for (i = i0; i < i1; ++i) {
    dfs3.dfs(Pinv[Bj[i]], Ai, Aj, x, xj, Pinv);
  }
  for (i = xj.length - 1; i !== -1; --i) {
    x[xj[i]] = 0;
  }
...
numeric.ccsLUP1 = function ccsLUP1(A2, threshold) {
  ...
  var dfs3 = new numeric.ccsDFS(m);
  if (typeof threshold === "undefined") {
    threshold = 1;
  }
  for (i = 0; i < m; ++i) {
    sol(L, A2, x, xj, i, Pinv, dfs3);
    a = -1;
    e = -1;
...
numeric.ccsDFS0.prototype.dfs = function dfs2(J, Ai, Aj, x, xj, Pinv, P) {
  var m = 0, foo, n = xj.length;
  var k2 = this.k, k1 = this.k1, j = this.j, km, k11;
  if (x[J] !== 0) return;
  x[J] = 1;
...
numeric.ccsLPSolve0 = function ccsLPSolve0(A2, B2, y, xj, I, Pinv, P, dfs3) {
  var Ai = A2[0], Aj = A2[1], Av = A2[2], m = Ai.length - 1, n = 0;
  var Bi = B2[0], Bj = B2[1], Bv = B2[2];
  var i, i0, i1, j, J, j0, j1, k2, l, l0, l1, a;
  i0 = Bi[I];
  ...
  xj.length = 0;
  for (i = i0; i < i1; ++i) {
    dfs3.dfs(Bj[i], Ai, Aj, y, xj, Pinv, P);
  }
  for (i = xj.length - 1; i !== -1; --i) {
    j = xj[i];
    y[P[j]] = 0;
...
numeric.ccsLUP0 = function ccsLUP0(A2, threshold) {
  ...
  var P = numeric.linspace(0, m - 1), Pinv = numeric.linspace(0, m - 1);
  var dfs3 = new numeric.ccsDFS0(m);
  if (typeof threshold === "undefined") {
    threshold = 1;
  }
  for (i = 0; i < m; ++i) {
    sol(L, A2, y, xj, i, Pinv, P, dfs3);
    a = -1;
    e = -1;
    for (j = xj.length - 1; j !== -1; --j) {
      k2 = xj[j];
...
numeric.ccsLUPSolve = function ccsLUPSolve(LUP2, B2) {
  var L = LUP2.L, U = LUP2.U, P = LUP2.P;
  var Bi = B2[0];
  var flag = false;
  if (typeof Bi !== "object") {
...
numeric.sdim = function dim2(A2, ret, k2) {
  ...
  var i;
  for (i in A2) {
    if (A2.hasOwnProperty(i)) dim2(A2[i], ret, k2 + 1);
  }
  return ret;
};
...
numeric.sdiag = function diag3(d) {
...
numeric.stranspose = function transpose3(A2) {
...
numeric.sdotMV = function dotMV2(A2, x) {
...
numeric.sdotVM = function dotMV3(x, A2) {
  var i, j, Ai, alpha;
  var ret = [], accum;
  for (i in x) {
...
numeric.sdotVV = function dotVV2(x, y) {
  var i, ret = 0;
  for (i in x) {
...
numeric.sdot = function dot2(A2, B2) {
...
numeric.cdotMV = function dotMV4(A2, x) {
...
numeric.Spline.prototype._at = function _at(x1, p) {
  ...
  t2 = (x1 - x[p]) / (x[p + 1] - x[p]);
  var s = t2 * (1 - t2);
  return add(add(add(mul(1 - t2, yl[p]), mul(t2, yr[p + 1])), mul(a, s * (1 - t2))), mul(b, s * t2));
};
numeric.Spline.prototype.at = function at(x0) {
  if (typeof x0 === "number") {
    var x = this.x;
    var n = x.length;
    var p, q, mid, floor = Math.floor, a, b, t2;
    p = 0;
    q = n - 1;
    while (q - p > 1) {
      mid = floor((p + q) / 2);
...
numeric.Spline.prototype.diff = function diff() {
  ...
  var add = numeric.add, mul = numeric.mul, div2 = numeric.div, sub = numeric.sub;
  for (i = n - 1; i !== -1; --i) {
    dx = x[i + 1] - x[i];
    dy = sub(yr[i + 1], yl[i]);
    pl[i] = div2(add(mul(dy, 6), mul(kl[i], -4 * dx), mul(kr[i + 1], -2 * dx)), dx * dx);
    pr[i + 1] = div2(add(mul(dy, -6), mul(kl[i], 2 * dx), mul(kr[i + 1], 4 * dx)), dx * dx);
  }
...
numeric.Spline.prototype.roots = function roots() {
  ...
  function heval(y02, y12, k02, k12, x2) {
    ...
    var t3 = (x2 + 1) * 0.5;
    var s2 = t3 * (1 - t3);
    return (1 - t3) * y02 + t3 * y12 + A3 * s2 * (1 - t3) + B3 * s2 * t3;
  }
  var ret = [];
  ...
  var m = yl.length, n = x.length - 1, i, j, k2, y, s, t2;
...
numeric.spline = function spline(x, y, k1, kn) {
  ...
  switch (typeof k1) {
    case "undefined":
      ...
      T3[0].push(0, 0);
      T3[1].push(0, 1);
      T3[2].push(2 / dx[0], 1 / dx[0]);
      break;
    case "string":
      b[0] = add(mul(3 / (dx[n - 2] * dx[n - 2]), dy[n - 2]), mul(3 / (dx[0] * dx[0]), dy[0]));
      T3[0].push(0, 0, 0);
      T3[1].push(n - 2, 0, 1);
      T3[2].push(1 / dx[n - 2], 2 / dx[n - 2] + 2 / dx[0], 1 / dx[0]);
      break;
    default:
      b[0] = k1;
      T3[0].push(0);
  ...
  for (i = 1; i < n - 1; i++) {
    ...
    T3[0].push(i, i, i);
    T3[1].push(i - 1, i, i + 1);
    T3[2].push(1 / dx[i - 1], 2 / dx[i - 1] + 2 / dx[i], 1 / dx[i]);
  }
  switch (typeof kn) {
    case "undefined":
      ...
      T3[0].push(n - 1, n - 1);
      T3[1].push(n - 2, n - 1);
      T3[2].push(1 / dx[n - 2], 2 / dx[n - 2]);
      break;
    case "string":
      T3[1][T3[1].length - 1] = 0;
      break;
    default:
      b[n - 1] = kn;
      T3[0].push(n - 1);
  ...
  if (typeof k1 === "string") {
    for (i = k2.length - 1; i !== -1; --i) {
      k2[i] = numeric.ccsLUPSolve(numeric.ccsLUP(numeric.ccsScatter(T3)), b[i]);
      k2[i][n - 1] = k2[i][0];
    }
  } else {
    for (i = k2.length - 1; i !== -1; --i) {
      k2[i] = numeric.cLUsolve(numeric.cLU(T3), b[i]);
    }
  }
...
numeric.fftpow2 = function fftpow2(x, y) {
  ...
  fftpow2(xo, yo);
  j = n / 2;
  var t2, k2 = -6.283185307179586 / n, ci, si;
  for (i = n - 1; i !== -1; --i) {
    --j;
    if (j === -1) j = n / 2 - 1;
    t2 = k2 * i;
...
numeric.T.prototype.fft = function fft() {
  ...
  var k2, c = -3.141592653589793 / n, t2;
...
numeric.gradient = function gradient(f, x) {
  ...
  var div2 = numeric.div, sub = numeric.sub, errest, roundoff, max = Math.max, eps = 1e-3, abs = Math.abs, min = Math.min;
...
numeric.uncmin = function uncmin(f, x0, tol, gradient2, maxit, callback, options) {
  ...
  var max = Math.max, norm22 = numeric.norm2;
  tol = max(tol, numeric.epsilon);
  var step, g0, g1, H1 = options.Hinv || numeric.identity(n);
  var dot3 = numeric.dot, inv3 = numeric.inv, sub = numeric.sub, add = numeric.add, ten = numeric.tensor, div2 = numeric.div, mul = numeric.mul;
  var all = numeric.all, isfinite = numeric.isFinite, neg = numeric.neg;
  var it = 0, i, s, x1, y, Hy, Hs, ys, i0, t2, nstep, t1, t22;
  var msg = "";
  g0 = gradient2(x0);
  while (it < maxit) {
...
numeric.Dopri.prototype._at = function _at2(xi, j) {
  function sqr(x) {
    return x * x;
  }
  var sol = this;
...
numeric.Dopri.prototype.at = function at2(x) {
...
numeric.LUsolve = function LUsolve2(LUP2, b) {
  var i, j;
  var LU2 = LUP2.LU;
  var n = LU2.length;
...
numeric.__solveLP = function __solveLP(c, A2, b, tol, maxit, x, flag) {
  var sum = numeric.sum, log = numeric.log, mul = numeric.mul, sub = numeric.sub, dot3 = numeric.dot, div2 = numeric.div, add = numeric.add;
  var m = c.length, n = b.length, y;
  var unbounded = false, cb, i0 = 0;
  var alpha = 1;
  var f0, df0, AT = numeric.transpose(A2), svd = numeric.svd, transpose4 = numeric.transpose, leq = numeric.leq, sqrt = Math.sqrt, abs = Math.abs;
  var muleq = numeric.muleq;
  var norm = numeric.norminf, any = numeric.any, min = Math.min;
  var all = numeric.all, gt = numeric.gt;
  var p = Array(m), A0 = Array(n), e = numeric.rep([n], 1), H;
  var solve2 = numeric.solve, z = sub(b, dot3(A2, x)), count;
  var dotcc = dot3(c, c);
  var g;
  for (count = i0; count < maxit; ++count) {
    var i, j, d;
    for (i = n - 1; i !== -1; --i) A0[i] = div2(A2[i], z[i]);
    var A1 = transpose4(A0);
    for (i = m - 1; i !== -1; --i) p[i] = /*x[i]+*/
    sum(A1[i]);
    alpha = 0.25 * abs(dotcc / dot3(c, p));
    var a1 = 100 * sqrt(dotcc / dot3(p, p));
    if (!isFinite(alpha) || alpha > a1) alpha = a1;
    g = add(c, mul(alpha, p));
    H = dot3(A1, A0);
    for (i = m - 1; i !== -1; --i) H[i][i] += 1;
    d = solve2(H, div2(g, alpha), true);
    var t0 = div2(z, dot3(A2, d));
    var t2 = 1;
    for (i = n - 1; i !== -1; --i) if (t0[i] < 0) t2 = min(t2, -0.999 * t0[i]);
    y = sub(x, mul(d, t2));
    z = sub(b, dot3(A2, y));
...
numeric._solveLP = function _solveLP(c, A2, b, tol, maxit) {
  ...
  x.length = m;
  var foo = numeric.inf(sub(b, dot3(A2, x)));
  if (foo < 0) {
...
numeric.solveLP = function solveLP(c, A2, b, Aeq, beq, tol, maxit) {
  ...
  var Aeq2 = g(Aeq, I, Q), A1 = g(A2, J, P), A22 = g(A2, J, Q), dot3 = numeric.dot, sub = numeric.sub;
  var A3 = dot3(A1, B2.I);
  var A4 = sub(A22, dot3(A3, Aeq2)), b4 = sub(b, dot3(A3, beq));
  var c1 = Array(P.length), c2 = Array(Q.length);
  for (i = P.length - 1; i !== -1; --i) c1[i] = c[P[i]];
  for (i = Q.length - 1; i !== -1; --i) c2[i] = c[Q[i]];
  var c4 = sub(c2, dot3(c1, dot3(B2.I, Aeq2)));
  var S = numeric._solveLP(c4, A4, b4, tol, maxit);
  var x2 = S.solution;
  if (x2 !== x2) return S;
  var x1 = dot3(B2.I, sub(beq, dot3(Aeq2, x2)));
...
var numeric_default = numeric;

// ../../node_modules/jsquest-plus/src/jsQuestPlus.js
console.log("jsQuestPlus Version 2.0.0");
var _jsquest = class _jsquest {
  ...
  constructor(settings) {
    ...
    this.comb_stim_params = stim_params.reduce(_jsquest.combvec);
    this.comb_PF_params = PF_params.reduce(_jsquest.combvec);
    let prior_data;
    if (typeof settings.priors === "undefined") {
      prior_data = _jsquest.set_prior(num_of_psy_samples);
    } else {
    ...
    this.posteriors = prior_data;
    this.responses = numeric_default.linspace(0, PF.length - 1, PF.length);
    ...
    this.PF.forEach((func) => {
      ...
      this.comb_stim_params.forEach((stim) => {
        const probabilities = [];
        this.comb_PF_params.forEach((param) => {
    ...
    this.expected_entropies_by_stim = _jsquest.update_entropy_by_stim(this);
  }
  // This is similar to the QuestSd function
  // http://psychtoolbox.org/docs/QuestSd 
  getSDs() {
    const params = numeric_default.transpose(this.comb_PF_params);
    const sd = [];
    params.forEach((data) => {
      const tmp = numeric_default.mul(data, this.normalized_posteriors);
      const mean = numeric_default.sum(tmp);
      const p = numeric_default.sum(this.normalized_posteriors);
      if (p === 0) {
        alert("Divided by zero.");
        console.error("Divided by zero.");
        this.nAlerts++;
      }
      const tmp1 = numeric_default.pow(data, 2);
      const tmp2 = numeric_default.mul(this.normalized_posteriors, tmp1);
      const tmp3 = numeric_default.div(numeric_default.sum(tmp2), p);
      const tmp4 = numeric_default.div(mean, p);
      const tmp5 = numeric_default.pow(tmp4, [2]);
      const tmp6 = numeric_default.sqrt(numeric_default.sub(tmp3, tmp5));
      sd.push(tmp6[0]);
    });
    return sd;
  }
    ...
    switch (thresholdingRule) {
      case "mean": {
        ...
        params.forEach((data) => {
          ...
          deviation.push(numeric_default.pow(numeric_default.sub(data, mean), 2));
        });
        if (roundStimuliToDomain) {
          const dev_matrix = numeric_default.transpose(deviation);
          const tmp_array = [];
          dev_matrix.forEach((data) => {
            const avg = numeric_default.sum(data) / data.length;
            tmp_array.push(Math.sqrt(avg));
          });
          const idx_mean = _jsquest.find_min_index(tmp_array);
      ...
      case "mode": {
        const idx_mode = _jsquest.find_max_index(this.normalized_posteriors);
  ...
  getStimParams(num) {
    ...
    if (typeof num === "undefined" || num === 1) {
      index = _jsquest.find_min_index(this.expected_entropies_by_stim);
    } else {
      const tmp_array = numeric_default.linspace(0, this.expected_entropies_by_stim.length - 1);
  ...
  update(stim, resp) {
    ...
    const new_posterior = numeric_default.mul(this.normalized_posteriors, this.precomputed_outcome_proportions[resp][stimIdx]);
    const s = numeric_default.sum(new_posterior);
    if (s === 0) {
      alert("Divided by zero.");
      console.error("Divided by zero.");
      this.nAlerts++;
    }
    this.normalized_posteriors = numeric_default.div(new_posterior, s);
    ...
    prior_array.forEach((prob_array) => {
      if (Array.isArray(prob_array)) {
        if (typeof norm_flag === "undefined" || norm_flag) {
          if (Math.abs(numeric_default.sum(prob_array) - 1) > 0.01) alert(`set_prior Error: The sum of the probability array is not 1. The sum is ${numeric_default.sum(prob_array)}.`);
        }
        priors.push(prob_array);
      } else {
        const unit_vector = numeric_default.linspace(1, 1, prob_array);
        if (prob_array === 0) {
          alert("Divided by zero.");
          console.error("set_prior Error 1: Divided by zero.");
        }
        priors.push(numeric_default.div(unit_vector, prob_array));
      }
    });
    const comb_priors = priors.reduce(_jsquest.combvec);
    let mulitiplied_priors = [];
    if (Array.isArray(comb_priors[0])) {
      comb_priors.forEach((element) => {
        mulitiplied_priors.push(element.reduce(_jsquest.multiply_reducer));
      });
    } else {
      mulitiplied_priors = comb_priors;
    }
    const sum_of_priors = numeric_default.sum(mulitiplied_priors);
    if (sum_of_priors === 0) {
      alert("Divided by zero.");
      console.error("set_prior Error 2: Divided by zero.");
    }
    const normalized_priors = numeric_default.div(mulitiplied_priors, sum_of_priors);
    return {
      priors,
      comb_priors,
      normalized_priors
  ...
  static update_entropy_by_stim(data) {
    const EH_array = numeric_default.rep([data.comb_stim_params.length], 0);
    data.precomputed_outcome_proportions.forEach((proportions_at_stim_params) => {
      proportions_at_stim_params.forEach((proportions_at_PF_params, index) => {
        const posterior_times_proportions = numeric_default.mul(data.normalized_posteriors, proportions_at_PF_params);
        const expected_outcomes = numeric_default.sum(posterior_times_proportions);
        ...
        const posterior = numeric_default.div(posterior_times_proportions, expected_outcomes);
        const tmp_entropy = numeric_default.mul(posterior, _jsquest.log2(posterior));
        const H = -1 * tmp_entropy.reduce((a, b) => a + (isNaN(b) ? 0 : b), 0);
  ...
  static abs(a) {
    return numeric_default.abs(a);
  }
  static add(a, b) {
    return numeric_default.add(a, b);
  }
  static cos(a) {
    return numeric_default.cos(a);
  }
  static dim(a) {
    return numeric_default.dim(a);
  }
  static div(a, b) {
    return numeric_default.div(a, b);
  }
  static exp(a) {
    return numeric_default.exp(a);
  }
  static floor(a) {
    return numeric_default.floor(a);
  }
  static isFinite(a) {
    return numeric_default.isFinite(a);
  }
  static isNaN(a) {
    return numeric_default.isNaN(a);
  }
  static log(a) {
    return numeric_default.log(a);
  }
  static mod(a, b) {
    return numeric_default.mod(a, b);
  }
  static mul(a, b) {
    return numeric_default.mul(a, b);
  }
  static pow(a, b) {
    return numeric_default.pow(a, b);
  }
  static round(a) {
    return numeric_default.round(a);
  }
  static sin(a) {
    return numeric_default.sin(a);
  }
  static sqrt(a) {
    return numeric_default.sqrt(a);
  }
  static sub(a, b) {
    return numeric_default.sub(a, b);
  }
  static sum(a) {
    return numeric_default.sum(a);
  }
  static log2(array) {
  ...
  static combvec(a, b, divide_flag) {
    ...
    if (Array.isArray(a) && divide_flag) {
      divide_flag = false;
      a.forEach((element) => {
        const res = _jsquest.combvec(element, b, divide_flag);
        output = output.concat(res);
      });
    } else if (Array.isArray(b)) {
      b.forEach((element) => {
        output.push(_jsquest.combvec(a, element, divide_flag));
      });
    } else {
  ...
  static normcdf(x, mean, sigma) {
    ...
    const erf = 1 - ((((a5 * t2 + a4) * t2 + a3) * t2 + a2) * t2 + a1) * t2 * Math.exp(-z * z);
    let sign = 1;
    if (z < 0) {
      sign = -1;
    }
  ...
  static simulate_weibull_two_resp(current_intensity, true_threshold, true_slope, true_guess, true_lapse) {
    if (Math.random() > _jsquest.weibull(current_intensity, true_threshold, true_slope, true_guess, true_lapse)) {
      return 1;
    } else {
      return 0;
    }
  ...
  static array(start, interval, end) {
    ...
    return numeric_default.linspace(start, adjusted_end, tmp + 1);
  }
  // exp((-1/2) * ((x_array - mean)/sd)^2)
  static gauss(x_array, mean, sd, norm_flag) {
    const tmp1 = numeric_default.div(numeric_default.sub(x_array, mean), sd);
    const tmp2 = numeric_default.pow(tmp1, 2);
    const tmp3 = numeric_default.exp(numeric_default.mul(-0.5, tmp2));
    if (typeof norm_flag === "undefined" || norm_flag) {
      return numeric_default.div(tmp3, numeric_default.sum(tmp3));
    } else {
      return tmp3;
    }
  }
...
__publicField(_jsquest, "linspace", numeric_default.linspace);
__publicField(_jsquest, "dot", numeric_default.dot);
__publicField(_jsquest, "transpose", numeric_default.transpose);
var jsquest = _jsquest;
var jsQuestPlus_default = jsquest;
export {
  jsQuestPlus_default as default
};
//# sourceMappingURL=jsquest-plus.js.map
