...
async function runChangeDetectionTask(context) {
    ...
    return orchestrator.run({
        ...
        runTrial: async ({ trial }) => {
            ...
            const result = await runChangeDetectionTrial(trial, taskConfig, sceneRenderer, ctx, container, layout, feedbackConfig, rng);
        ...
        onBlockEnd: ({ blockIndex, trialResults }) => {
            const block = asObject(blocks[blockIndex]);
            const correct = trialResults.reduce((acc, r) => acc + (r?.responseCorrect ?? 0), 0);
        ...
        csvOptions: {
            ...
            getRecords: (sessionResult) => sessionResult.blocks.flatMap((b) => b.trialResults.map((r) => ({
...
async function runChangeDetectionTrial(trial, config, renderer, ctx, container, layout, feedbackConfig, rng) {
