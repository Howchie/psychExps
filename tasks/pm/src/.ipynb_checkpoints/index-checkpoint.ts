  ...
  for (const block of plan) {
    ...
    for (let instructionIndex = 0; instructionIndex < block.beforeBlockScreens.length; instructionIndex += 1) {
      ...
      pushJsPsychContinueScreen(
        ...
        CallFunctionPlugin,
        root,
        `<h3>${escapeHtml(block.label)}</h3><p>${escapeHtml(instruction)}</p>`,
...
function parseBlock(
  ...
  const resolvedBeforeBlockScreens = variableResolver ? variableResolver.resolveToken(rawBeforeBlockScreens, scope) : rawBeforeBlockScreens;
  const resolvedAfterBlockScreens = variableResolver ? variableResolver.resolveToken(rawAfterBlockScreens, scope) : rawAfterBlockScreens;
