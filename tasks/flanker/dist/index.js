import { asObject, asString, computeCanvasFrameLayout, computeRtPhaseDurations, createConditionCellId, createEventLogger, createMulberry32, createResponseSemantics, drawCanvasCenteredText, drawCanvasFramedScene, drawTrialFeedbackOnCanvas, escapeHtml, evaluateTrialOutcome, hashSeed, TaskEnvironmentGuard, normalizeKey, parseTrialFeedbackConfig, buildTaskInstructionConfig, resolveTrialFeedbackView, runJsPsychTimeline, setCursorHidden, shouldHideCursorForPhase, computeAccuracy, TaskOrchestrator, createInstructionRenderer, toJsPsychChoices, toNonNegativeNumber, toPositiveNumber, toStringScreens, ensureJsPsychCanvasCentered, applyTaskInstructionConfig, createTaskAdapter, buildJsPsychRtTimelineNodes, resolveRtTaskConfig, buildConditionSequence, maybeExportStimulusRows, } from "@experiments/core";
...
async function runFlankerTask(context) {
    ...
    const payload = await orchestrator.run({
        ...
        runTrial: async ({ block, blockIndex, trial }) => {
            ...
            if (!record)
                throw new Error(`Flanker record not found for block ${blockIndex} trial ${trial.trialIndex}`);
            return record;
        },
        onTaskStart: () => {
            jsPsych = initJsPsych({
...
export async function parseFlankerConfig(config) {
    ...
    const labels = ["congruent", "incongruent", "neutral"];
    ...
    if (flankerCount % 2 !== 0)
        flankerCount += 1; // ensure even number of flankers to balance
    return {
        title: asString(taskRaw?.title) || "Flanker Task",
...
function buildFlankerPlan(parsed, baseRng) {
    const output = [];
...
function appendFlankerTrialTimeline(args) {
    ...
    const nodes = buildJsPsychRtTimelineNodes({
        ...
        renderFixation: (canvas) => drawFixation(canvas, parsed, layout),
        renderBlank: (canvas) => drawBlank(canvas, parsed, layout),
        renderStimulus: (canvas) => drawStimulus(canvas, parsed, layout, trial),
        renderFeedback: (canvas) => drawFeedback(canvas, parsed, layout, feedback, state.feedbackView),
        feedback: {
