...
async function runStroopTask(context) {
    ...
    const payload = await orchestrator.run({
        ...
        runTrial: async ({ block, blockIndex, trial }) => {
            ...
            if (!record)
                throw new Error(`Stroop record not found for block ${blockIndex} trial ${trial.trialIndex}`);
            return record;
        },
        onTaskStart: () => {
            jsPsych = initJsPsych({
