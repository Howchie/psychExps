...
export class GameState {
    ...
    _resolveCategorySelectionsForEntry(entry) {
        ...
        return {
            color: this._resolveCategoryById('color', sampledId(['colorCategoryId', 'color_category_id']) ?? categoryIds.color),
            width: this._resolveCategoryById('width', sampledId(['widthCategoryId', 'width_category_id']) ?? categoryIds.width),
            borderColor: this._resolveCategoryById('borderColor', sampledId(['borderColorCategoryId', 'border_color_category_id', 'borderColor_category_id']) ?? categoryIds.borderColor),
            shape: this._resolveCategoryById('shape', sampledId(['shapeCategoryId', 'shape_category_id']) ?? categoryIds.shape),
            texture: this._resolveCategoryById('texture', sampledId(['textureCategoryId', 'texture_category_id']) ?? categoryIds.texture)
        };
    }
    ...
    handleBrickHold(brickId, holdDurationMs, timestamp, clickPos = {}) {
        ...
        const progressPerPerfect = Math.max(0.01, Math.min(1, Number(brick.progressPerPerfect ?? params.progress_per_perfect ?? 0.35)));
