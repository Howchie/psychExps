...
const materializeCompletePreset = (preset) => {
    ...
    if (isObject(bg) && String(bg.renderMode ?? '').toLowerCase() === 'procedural_warehouse') {
        out.backgroundTexture = {
            ...bg,
            proceduralWarehouse: deepMerge(COMPLETE_WAREHOUSE_PROCEDURAL_DEFAULTS, isObject(bg.proceduralWarehouse) ? bg.proceduralWarehouse : {}),
        };
    }
    ...
    if (isObject(belt) && String(belt.renderMode ?? '').toLowerCase() === 'procedural_topdown') {
        out.beltTexture = {
            ...belt,
            proceduralTopdown: deepMerge(COMPLETE_BELT_PROCEDURAL_DEFAULTS, isObject(belt.proceduralTopdown) ? belt.proceduralTopdown : {}),
        };
    }
    return out;
};
...
//# sourceMappingURL=display_presets.js.map