...
export class ConveyorRenderer {
    ...
    _drawProceduralTopdownBeltGraphics(target, { beltLength, beltHeight, phaseX = 0, styleCfg = {}, styleScaleX = 1, styleScaleY = 1 } = {}) {
        if (!target) {
            return;
        }
    ...
    _drawBelts() {
        ...
        for (let i = 0; i < n; i += 1) {
            ...
            const sampledLength = runtimeLengths && Number.isFinite(runtimeLengths[i])
                ? runtimeLengths[i]
                : fallbackLength;
    ...
    _getInteractionTargetMode() {
        ...
        const explicitMode = String(interactionCfg.targetingArea ?? interactionCfg.targetArea ?? interactionCfg.hitAreaMode ?? '').trim().toLowerCase();
    ...
    _syncSpotlightHoverTarget(completionMode) {
        if (!this._isSpotlightHitAreaEnabled() || completionMode !== 'hover_to_clear') {
            if (this.spotlightHoveredBrickId) {
                this.onBrickHover(this.spotlightHoveredBrickId, false, this.spotlightPointerPos?.x ?? null, this.spotlightPointerPos?.y ?? null);
                this.spotlightHoveredBrickId = null;
            }
            return;
        }
    ...
    updateBelts(conveyors, dtMs) {
        ...
        const hasAnimatedBelts = this.beltVisuals.some((vis) => vis?.type === 'tiling' || vis?.type === 'procedural_redraw');
        if (!hasAnimatedBelts) {
            return;
        }
    ...
    updateFurnaces(dtMs) {
        ...
        this.furnaceVisuals.forEach((vis) => {
            ...
            const intensity = Math.max(0.05, Math.min(1, baseIntensity + (blend - 0.5) * 2 * alphaAmplitude + jitter));
    ...
    _resolveHudCoinSize(text, uiCfg, clearCfg) {
        ...
        const lineHeight = Math.max(8, Number(uiCfg?.hudLineHeight ?? text?.style?.lineHeight ?? text?.style?.fontSize ?? 16));
        let capHeight = 0;
        try {
    ...
    queueClearEffects(clearEvents = []) {
        ...
        clearEvents.forEach((entry) => {
            ...
            const textSize = Math.max(clearCfg.textMinSizePx, Math.min(clearCfg.textMaxSizePx, height * clearCfg.textSizeFactor));
    ...
    queueDropEffects(dropEvents = []) {
        ...
        dropEvents.forEach((drop) => {
            ...
            if (mode === 'furnace' && furnace) {
                ...
                glowFx.drawRect(furnace.mouthX + 1, furnace.mouthY + 1, Math.max(2, furnace.mouthWidth - 2), Math.max(2, furnace.mouthHeight - 2));
                glowFx.endFill();
                ...
                if (furnace) {
                    ...
                    mouthPulse.drawRect(furnace.mouthX + 1, furnace.mouthY + 1, Math.max(2, furnace.mouthWidth - 2), Math.max(2, furnace.mouthHeight - 2));
                    mouthPulse.endFill();
            ...
            if (marker) {
                ...
                flashFx.drawRoundedRect(Number(marker.x) - flashWidth / 2, Number(marker.y) - flashHeight / 2, flashWidth, flashHeight, Math.min(4, flashWidth / 2));
                flashFx.endFill();
                this.effectLayer.addChild(flashFx);
                this.effectVisuals.push({
                    kind: 'marker_flash',
    ...
    _drawBrickGraphics(sprite, brick, completionMode) {
        ...
        const cornerRadius = Math.max(0, Math.min(Number(defaultCornerRadius) || 0, Number(brick.width) / 2, Number(brick.height) / 2));
    ...
    _updateSpotlight(focusState) {
        ...
        if (typeof this.spotlightGraphics.beginHole === 'function' && typeof this.spotlightGraphics.endHole === 'function') {
            ...
            this.spotlightGraphics.drawRect(Math.max(0, holeX + holeW), Math.max(0, holeY), Math.max(0, canvasW - (holeX + holeW)), Math.max(0, holeH));
        }
