...
async function exportPmStimulusList(context: TaskAdapterContext, runtime: PmRuntimeState): Promise<unknown> {
...
async function runPmJsPsychTask(context: TaskAdapterContext, runtime: PmRuntimeState): Promise<void> {
  ...
  for (let introIndex = 0; introIndex < parsed.instructions.introPages.length; introIndex += 1) {
    pushJsPsychContinueScreen(
      ...
      CallFunctionPlugin,
      root,
      `<p>${escapeHtml(parsed.instructions.introPages[introIndex] ?? "")}</p>`,
      "intro_page",
      `pm-continue-intro_page-${introIndex}`,
      { introIndex },
    );
  }
  if (parsed.instructions.pmTemplate) {
    pushJsPsychContinueScreen(
      ...
      CallFunctionPlugin,
      root,
      `<p>${escapeHtml(parsed.instructions.pmTemplate.replace("{pmCategoryText}", parsed.pmCategories.join(", ")))}</p>`,
      "intro_pm_template",
      "pm-continue-intro_pm_template",
    );
  }
  ...
  for (const block of plan) {
    ...
    const pushPreBlockScreens = (): void => {
      for (let instructionIndex = 0; instructionIndex < preBlockScreens.length; instructionIndex += 1) {
        ...
        pushJsPsychContinueScreen(
          ...
          CallFunctionPlugin,
          root,
          blockScreenHtml(instruction),
          "block_pre_instruction",
          `pm-continue-block_pre_instruction-${block.blockIndex}-${instructionIndex}`,
          { blockIndex: block.blockIndex, instructionIndex },
        );
      }
    };
    const pushBlockIntro = (): void => {
      pushJsPsychContinueScreen(
        timeline,
        CallFunctionPlugin,
        root,
    ...
    timeline.push({
      ...
      func: (done: () => void) => {
        ...
        void waitForContinue(
          host,
          `<h3>${endTitle}</h3><p>Accuracy: <b>${accuracy.toFixed(1)}%</b></p><p>Responses: <b>${blockResponded}</b> / ${block.trials.length}</p>`,
    ...
    for (let instructionIndex = 0; instructionIndex < postBlockScreens.length; instructionIndex += 1) {
      ...
      pushJsPsychContinueScreen(
        ...
        root,
        blockScreenHtml(instruction),
        "block_post_instruction",
        `pm-continue-block_post_instruction-${block.blockIndex}-${instructionIndex}`,
        { blockIndex: block.blockIndex, instructionIndex },
      );
    }
  }
  for (let endIndex = 0; endIndex < parsed.instructions.endPages.length; endIndex += 1) {
    pushJsPsychContinueScreen(
      ...
      CallFunctionPlugin,
      root,
      `<p>${escapeHtml(parsed.instructions.endPages[endIndex] ?? "")}</p>`,
      "end_page",
      `pm-continue-end_page-${endIndex}`,
      { endIndex },
    );
  }
...
function resolveAllowedKeysForBlock(semantics: ResponseSemantics, blockType: "PM" | "Control"): string[] {
  if (blockType === "PM") return semantics.allowedKeys(["target", "non_target", "pm"]);
