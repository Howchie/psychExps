...
function parsePmConfig(config, selection, variableResolver) {
    ...
    const planPoolAllocator = createManipulationPoolAllocator(plan?.manipulationPools, selection
        ...
        : ["pm_plan_manipulation_pools"]);
    const mergedBlocks = asArray(plan?.blocks).map((entry, index) => parseBlock(applyManipulationOverridesToBlock(entry, resolveBlockManipulationIds(entry, planPoolAllocator), planManipulations, `Invalid PM plan: block ${index + 1}`), index, null, variableResolver, feedbackDefaults));
...
function parseBlock(entry, index, isPractice, variableResolver, feedbackFallback) {
    ...
    const activePmCategories = expandCategoryEntriesWithVariables(b.activePmCategories, ["pm"], variableResolver, scope);
    const nbackSourceCategories = expandCategoryEntriesWithVariables(b.nbackSourceCategories, ["other"], variableResolver, scope);
    const controlSourceCategories = expandCategoryEntriesWithVariables(b.controlSourceCategories, [], variableResolver, scope);
...
function expandCategoryEntriesWithVariables(entriesRaw, fallback, resolver, context) {
