...
## Phase 1: Core Spatial and Scene Enhancements
- [ ] Task: Implement `SpatialLayoutManager` in `@experiments/core`
    - [ ] Write unit tests for random and template-based non-overlapping slot generation
    - [ ] Implement `SpatialLayoutManager` with support for circular and grid templates
- [ ] Task: Implement `SceneStimulus` model and diffing logic
    - [ ] Write unit tests for scene creation and identity diffing (same/different)
    - [ ] Implement `SceneStimulus` types and `diffScenes` utility
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Core Spatial and Scene Enhancements' (Protocol in workflow.md)
